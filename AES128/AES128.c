/*  ======================================================================== *

                                    주 의 사 항


    1. 구현은 다양한 방식으로 이뤄질 수 있음
    2. AES128(...) 함수의 호출과 리턴이 여러번 반복되더라도 메모리 누수가 생기지 않게 함
    3. AddRoundKey 함수를 구현할 때에도 파라미터 rKey는 사전에 선언된 지역 배열을 가리키도록 해야 함
       (정확한 구현을 위해서는 포인터 개념의 이해가 필요함)
    4. 배열의 인덱스 계산시 아래에 정의된 KEY_SIZE, ROUNDKEY_SIZE, BLOCK_SIZE를 이용해야 함
       (상수 그대로 사용하면 안됨. 예로, 4, 16는 안되고 KEY_SIZE/4, BLOCK_SIZE로 사용해야 함)

 *  ======================================================================== */

#include <stdio.h>
#include <stdlib.h>
#include "AES128.h"

#define KEY_SIZE 16
#define ROUNDKEY_SIZE 176
#define BLOCK_SIZE 16

/* 기타 필요한 전역 변수 추가 선언 */

static const uint8_t rcon[256] = {
    };

static const uint8_t sbox[256] = {
    };

static const uint8_t inv_sbox[256] = {
    };

static const uint8_t mixCol[16] = {
    0x02, 0x03, 0x01, 0x01,
    0x01, 0x02, 0x03, 0x01,
    0x01, 0x01, 0x02, 0x03,
    0x03, 0x01, 0x01, 0x02};

static const uint8_t inv_mixCol[16] = {
    0x0E, 0x0B, 0x0D, 0x09,
    0x09, 0x0E, 0x0B, 0x0D,
    0x0D, 0x09, 0x0E, 0x0B,
    0x0B, 0x0D, 0x09, 0x0E};

/* 기타 필요한 함수 추가 선언 및 정의 */

BYTE gmul(uint8_t num, BYTE data) {
  BYTE tmp = 0;
  BYTE mask = 0x01;

  int i;
  for (i = 0;i < num;i ++){
      if (num & mask){
          tmp ^= data;
      }
      if (data & 0x80){
          data = (data << 1) ^ 0x1b;
      } else {
          data <<= 1;
      }
      mask <<= 1;
  }
  return tmp;
}

/*  <키스케줄링 함수>
 *
 *  key         키스케줄링을 수행할 16바이트 키
 *  roundKey    키스케줄링의 결과인 176바이트 라운드키가 담길 공간
 */
void expandKey(BYTE *key, BYTE *roundKey){
  /* 추가 구현 */
  int i, j, k;
  for(i=0; i<KEY_SIZE; i++){
    roundKey[i] = key[i];
  } //w0, w1, w2, w3

  uint8_t temp[KEY_SIZE/4];
  for(i=0; i<ROUNDKEY_SIZE/KEY_SIZE-1; i++){
    temp[0] = sbox[roundKey[13 + i*KEY_SIZE]];
    temp[1] = sbox[roundKey[14 + i*KEY_SIZE]];
    temp[2] = sbox[roundKey[15 + i*KEY_SIZE]];
    temp[3] = sbox[roundKey[12 + i*KEY_SIZE]];
    temp[0] ^= rcon[1+i]; //temp = g(w3+4i)

    for(j=0; j<4; j++){
      roundKey[(i+1)*KEY_SIZE + j] = temp[j] ^ roundKey[i*KEY_SIZE + j];
    }//w4, w8, w12, ...
    for(j=4; j<KEY_SIZE; j++){
      for(k=0; k<4; k++){
        roundKey[(i+1)*KEY_SIZE + j + k] = roundKey[j + (KEY_SIZE-4) + i*KEY_SIZE + k] ^ roundKey[j + i*KEY_SIZE + k];
      }
    }//나머지 w
  }
}


/*  <SubBytes 함수>
 *
 *  block   SubBytes 수행할 16바이트 블록. 수행 결과는 해당 배열에 바로 반영
 *  mode    SubBytes 수행 모드
 */
 BYTE* subBytes(BYTE *block, int mode){
    /* 필요하다 생각하면 추가 선언 */
    int i;

    switch(mode){

        case ENC:
            /* 추가 구현 */
            for(i=0; i<BLOCK_SIZE; i++){
              block[i] = sbox[block[i]];
            }
            break;

        case DEC:
            /* 추가 구현 */
            for(i=0; i<BLOCK_SIZE; i++){
              block[i] = inv_sbox[block[i]];
            }
            break;

        default:
            fprintf(stderr, "Invalid mode!\n");
            exit(1);
    }
    return block;
}


/*  <ShiftRows 함수>
 *
 *  block   ShiftRows 수행할 16바이트 블록. 수행 결과는 해당 배열에 바로 반영
 *  mode    ShiftRows 수행 모드
 */
BYTE* shiftRows(BYTE *block, int mode){

    /* 필요하다 생각하면 추가 선언 */
    int i;
    uint8_t temp[BLOCK_SIZE];

    switch(mode){

        case ENC:
            /* 추가 구현 */
            for(i=0; i<BLOCK_SIZE; i++){
              temp[i] = block[i];
            }
            block[1] = temp[5];
            block[2] = temp[10];
            block[3] = temp[15];
            block[5] = temp[9];
            block[6] = temp[14];
            block[7] = temp[3];
            block[9] = temp[13];
            block[10] = temp[2];
            block[11] = temp[7];
            block[13] = temp[1];
            block[14] = temp[6];
            block[15] = temp[11];
            break;

        case DEC:
            /* 추가 구현 */
            for(i=0; i<BLOCK_SIZE; i++){
              temp[i] = block[i];
            }
            block[1] = temp[13];
            block[2] = temp[10];
            block[3] = temp[7];
            block[5] = temp[1];
            block[6] = temp[14];
            block[7] = temp[11];
            block[9] = temp[5];
            block[10] = temp[2];
            block[11] = temp[15];
            block[13] = temp[9];
            block[14] = temp[6];
            block[15] = temp[3];
            break;

        default:
            fprintf(stderr, "Invalid mode!\n");
            exit(1);
    }
    return block;
}


/*  <MixColumns 함수>
 *
 *  block   MixColumns을 수행할 16바이트 블록. 수행 결과는 해당 배열에 바로 반영
 *  mode    MixColumns의 수행 모드
 */
BYTE* mixColumns(BYTE *block, int mode){

    /* 필요하다 생각하면 추가 선언 */
    int i, j;
    BYTE temp[BLOCK_SIZE];

    switch(mode){

        case ENC:
            /* 추가 구현 */
            for(i=0; i<BLOCK_SIZE; i+=4){
                for(j=0; j<BLOCK_SIZE; j+=4){
                    temp[i+j/4] = gmul(mixCol[j], block[i]) ^ gmul(mixCol[j+1], block[i+1]) ^ gmul(mixCol[j+2], block[i+2]) ^ gmul(mixCol[j+3], block[i+3]);
                }
            }
            for(i=0; i<BLOCK_SIZE; i++){
                block[i] = temp[i];
            }
            break;

        case DEC:
            /* 추가 구현 */
            for(i=0; i<BLOCK_SIZE; i+=4){
                for(j=0; j<BLOCK_SIZE; j+=4){
                    temp[i+j/4] = gmul(inv_mixCol[j], block[i]) ^ gmul(inv_mixCol[j+1], block[i+1]) ^ gmul(inv_mixCol[j+2], block[i+2]) ^ gmul(inv_mixCol[j+3], block[i+3]);
                }
            }
            for(i=0; i<BLOCK_SIZE; i++){
                block[i] = temp[i];
            }
            break;

        default:
            fprintf(stderr, "Invalid mode!\n");
            exit(1);
    }
    return block;
}


/*  <AddRoundKey 함수>
 *
 *  block   AddRoundKey를 수행할 16바이트 블록. 수행 결과는 해당 배열에 반영
 *  rKey    AddRoundKey를 수행할 16바이트 라운드키
 */
BYTE* addRoundKey(BYTE *block, BYTE *rKey){
    /* 추가 구현 */
    int i;
    for(i=0; i<BLOCK_SIZE; i++){
      block[i] ^= rKey[i];
    }
    return block;
}


/*  <128비트 AES 암복호화 함수>
 *
 *  mode가 ENC일 경우 평문을 암호화하고, DEC일 경우 암호문을 복호화하는 함수
 *
 *  [ENC 모드]
 *  input   평문 바이트 배열
 *  output  결과(암호문)이 담길 바이트 배열. 호출하는 사용자가 사전에 메모리를 할당하여 파라미터로 넘어옴
 *  key     128비트 암호키 (16바이트)
 *
 *  [DEC 모드]
 *  input   암호문 바이트 배열
 *  output  결과(평문)가 담길 바이트 배열. 호출하는 사용자가 사전에 메모리를 할당하여 파라미터로 넘어옴
 *  key     128비트 암호키 (16바이트)
 */
void AES128(BYTE *input, BYTE *output, BYTE *key, int mode){

    BYTE roundKey[ROUNDKEY_SIZE];
    expandKey(key, roundKey);
    int i;

    if(mode == ENC){
        /* 추가 작업이 필요하다 생각하면 추가 구현 */
        addRoundKey(input, &roundKey[0]);

        for(i=1; i<=9; i++){
          subBytes(input, 1);
          shiftRows(input, 1);
          mixColumns(input, 1);
          addRoundKey(input, &roundKey[i*KEY_SIZE]);
        }

        subBytes(input, 1);
        shiftRows(input, 1);
        addRoundKey(input, &roundKey[10*KEY_SIZE]);
        for(i=0; i<BLOCK_SIZE; i++){
          output[i] = input[i];
        }
    }else if(mode == DEC){
        /* 추가 작업이 필요하다 생각하면 추가 구현 */
        addRoundKey(input, &roundKey[10*KEY_SIZE]);

        for(i=9; i>=1; i--){
          shiftRows(input, 0);
          subBytes(input, 0);
          addRoundKey(input, &roundKey[i*KEY_SIZE]);
          mixColumns(input, 0);
        }

        shiftRows(input, 0);
        subBytes(input, 0);
        addRoundKey(input, &roundKey[0]);
        for(i=0; i<BLOCK_SIZE; i++){
          output[i] = input[i];
        }
    }else{
        fprintf(stderr, "Invalid mode!\n");
        exit(1);
    }
}
